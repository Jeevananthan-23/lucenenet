  ...
  *//***//* Set to a non-null value to force exactly that many nodes; else, it's random. *//*
  ...
  readonly AtomicBoolean failed = new AtomicBoolean();
    ...
    *//***//* cwd where we start each child (server) node *//*
    ...
    readonly AtomicInt32 markerUpto = new AtomicInt32();
    readonly AtomicInt32 markerID = new AtomicInt32();

    *//***//* Maps searcher version to how many hits the query body:the matched. *//*
    readonly IDictionary<long, int> hitCounts = new ConcurrentDictionary<long, int>();

    *//***//* Maps searcher version to how many marker documents matched.  This should only ever grow (we never delete marker documents). *//*
    readonly IDictionary<long, int> versionToMarker = new ConcurrentDictionary< long, int>();

    *//***//* Maps searcher version to xlog location when refresh of this version started. *//*
    readonly IDictionary<long, long> versionToTransLogLocation = new ConcurrentDictionary<long, long>();

    readonly AtomicInt64 nodeStartCounter = new();

    readonly ISet<int> crashingNodes = Collections.EmptySet<int>();
    ...
    public void Test()
    {
        ...
        Node.Message("change thread name from " + Thread.CurrentThread.Name);
        Thread.CurrentThread.Name = "main";

        childTempDir = CreateTempDir("child").FullName;

        // We are parent process:

        // Silly bootstrapping:
        versionToTransLogLocation.Add(0L, 0L);

        versionToMarker.Add(0L, 0);

    int numNodes;

    if (NUM_NODES == null) {
            numNodes = TestUtil.NextInt32(Random, 2, 10);
        } else {
            numNodes = NUM_NODES.Value;
        }

            Console.WriteLine("TEST: using " + numNodes + " nodes");
    ...
    indexPaths = new string[numNodes];
    nodes = new NodeProcess[numNodes];
    nodeTimeStamps = new long[numNodes];
    Arrays.Fill(nodeTimeStamps, Node.globalStartNS);
    starting = new bool[numNodes];

    for(int i=0;i<numNodes;i++)
            {
      indexPaths[i] = CreateTempDir("index" + i).FullName;
    }

Thread[] indexers = new Thread[TestUtil.NextInt32(Random, 1, 3)];
Console.WriteLine("TEST: launch " + indexers.Length + " indexer threads");
for (int i = 0; i < indexers.Length; i++)
{
    indexers[i] = new IndexThread();
    indexers[i].Name = "indexer" + i;
    //indexers[i].setDaemon(true);
    indexers[i].Start();
}

Thread[] searchers = new Thread[TestUtil.NextInt32(Random, 1, 3)];
Console.WriteLine("TEST: launch " + searchers.Length + " searcher threads");
for (int i = 0; i < searchers.Length; i++)
{
    searchers[i] = new SearchThread();
    searchers[i].Name = "searcher" + i;
...
if (TestNightly)
{
    runTimeSec = RandomMultiplier * TestUtil.NextInt32(Random, 120, 240);
}
else
{
    runTimeSec = RandomMultiplier * TestUtil.NextInt32(Random, 45, 120);
}

Console.WriteLine("TEST: will run for " + runTimeSec + " sec");

long endTime = Time.NanoTime() + runTimeSec * 1000000000L;

sendReplicasToPrimary();

while (failed.Value == false && Time.NanoTime() < endTime)
{
    // Wait a bit:
    ThreadJob.Sleep(TestUtil.NextInt32(Random, Math.min(runTimeSec * 4, 200), runTimeSec * 4));
    if (primary != null && Random.nextBoolean())
    {
        ...
        if (curPrimary != null)
        {
            ...
            int markerUptoSav = markerUpto.Value;
    ...
    if (random().NextInt32(10) == 1)
    {
        NodeProcess node = nodes[random().NextInt32(nodes.Length)];
...
Console.WriteLine("TEST: top: now close replicas");
...
@SuppressForbidden(reason = "ProcessBuilder requires java.io.File for CWD")
  NodeProcess startNode(readonly int id, Path indexPath, bool isPrimary, long forcePrimaryVersion) throws IOException
{
    nodeTimeStamps [id] = Time.NanoTime();
...
readonly bool finalWillCrash = willCrash;
readonly AtomicBoolean nodeIsClosing = new AtomicBoolean();
...
{
    //Console.WriteLine("ADD VERSION MARKER version=" + version + " count=" + count);
    ...
    public void run()
{
    ...
    try
    {
        while (stop.get() == false)
        {
            ...
            if (DO_FULL_CLUSTER_CRASH && random().NextInt32(500) == 17)
            {
            ...
            {
                int idx = downNodes.get(random().NextInt32(downNodes.size()));
        ...
        Console.WriteLine("Restarter: now stop: join " + startupThreads.size() + " startup threads");
    ...
    public void run()
{
    ...
    while (stop.get() == false)
    {
        ...
        bool nodeIsPrimary = node == primary;

        try
        {
            Thread.currentThread().setName("Searcher node=" + node);

            //Console.WriteLine("S: cycle; conns=" + connections);
            ...
            try
            {
                if (c == null)
                {
                    //Console.WriteLine("S: new connection " + node.id + " " + Thread.currentThread().getName());
                    c = new Connection(node.tcpPort);
                    connections.put(node.id, c);
                }
                else
                {
                    //Console.WriteLine("S: reuse connection " + node.id + " " + Thread.currentThread().getName());
                }
            ...
            if (expectedAtLeastHitCount != null && expectedAtLeastHitCount > 0 && random().NextInt32(10) == 7)
            {
                try
                {
    ...
    Console.WriteLine("Searcher: now stop");
    IOUtils.closeWhileHandlingException(connections.values());
}
  }

  private class IndexThread : ThreadJob
{
   //// @Override
    public void run()
{
    try
    {
        ...
        int sleepChance = TestUtil.NextInt32(Random, 4, 100);
        ...
        while (stop.get() == false)
        {
            try
            {
                ...
                {
                    int randomID = random().NextInt32(docCount);
            ...
            if (random().NextInt32(sleepChance) == 0)
            {
                Thread.sleep(10);
            }

            if (random().NextInt32(100) == 17)
            {
                int pauseMS = TestUtil.NextInt32(Random, 500, 2000);
                Console.WriteLine("Indexer: now pause for " + pauseMS + " msec...");
                Thread.sleep(pauseMS);
                Console.WriteLine("Indexer: done pause for a bit...");
            }
        }
        if (curPrimary != null)
        {
        ...
        Console.WriteLine("Indexer: now stop");
    }
    catch (Throwable t)
    {
        failed.set(true);
  ...
  static void message(String message)
{
    ...
    Console.WriteLine(String.format(Locale.ROOT,
                                     "%5.3fs       :     parent [%11s] %s",
